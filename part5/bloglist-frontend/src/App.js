...
const App = () => {
    ...
    const addBlog = async (blogObject) => {
        try {
            ...
            displayMessage(`Added new blog "${blogObject.title}" by ${blogObject.author}`, 'message')
        } catch (exception) {
            displayMessage(`Unable to add blog "${blogObject.title}" by ${blogObject.author}`, 'error')
        }
    }
    ...
    const updateBlog = async (blogObject) => {
        try {
            ...
            displayMessage(`Added like to "${blogObject.title}" by ${blogObject.author}`, 'message')
        } catch (exception) {
            displayMessage(`Unable to add like to "${blogObject.title}" by ${blogObject.author}`, 'error')
        }
    }
    ...
    const deleteBlog = async (blogObject) => {
        if (window.confirm(`Delete "${blogObject.title}" by ${blogObject.author}?`)) {
            try {
                ...
                displayMessage(`Deleted "${blogObject.title}" by ${blogObject.author}`, 'message')
            } catch (exception) {
                displayMessage(`Unable to delete "${blogObject.title}" by ${blogObject.author}`, 'error')
            }
        }
    }
